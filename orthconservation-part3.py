...
for combo in orthin:
	...
	for m1,m2 in zip(c1,c2):
		...
		if m1[0] == '{' or m2[0] == '{':
			...
			if m1 == m2: #not perfect, because what if an extra element is added in one? then ambiguity is conserved in part!
				score_con['ambi'] += 1
			else:
				score_non['ambi'] += 1
...
# save per motif:
# the number of times it was recorded in combination with the same motif (counted twice!)
# the number of times it was recorded with another motif
